...
In~\cite{eproofs-alcock+wilkinson}, Alcock and Wilkinson present a tool for walking through real analysis proofs. Users are able to step through a proof and (TODO: icons? ..) are used to bring the reader's attention to the current point in the proof, as well as link to dependencies elsewhere in the proof. (TODO: more? how is this related to this work?)
...
The first is in the form of a proof walk-through that will support operational comprehension by using colour to focus attention on the current inference in a proof and distinguish between previously seen inferences and future inferences. Past inferences are coloured to be less visible. Another panel on the page will have information on how the focused node is derived to provide discourse. (TODO: adjust this argument and bring out content on colour and panel because it applies to both modes)

The second form of interaction will allow more free-form exploration of the proof tree. The user can click on any node to focus on it.
...
A difference to note between the lecture notes and the presentation here is we do not make an explicit distinction between a formula and the judgment that the formula is true. For example, whenever we see a formula $P$ in a proof, we assume that this means $P$ is true, rather than writing $P \; true$ directly in the proof tree. Here we prefer the more condensed presentation and don't see the more careful explicit presentation as necessary to achieve our goals of making this tool accessible to beginners. Advanced users can be more careful when writing their own proof trees using the tool. (TODO: more this difference discussion to Discussion section?)
...
\begin{itemize}
    ...
    \item reasoning direction (forward more natural, but backward more conducive to automated reasoning and more standard reading for logic programs; unambiguous application of rules)
